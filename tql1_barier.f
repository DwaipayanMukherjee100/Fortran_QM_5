c--- we shall now discretize TISE and use tridiagonalization 
c---- subroutine TQL2 to find eigen values and eigen functions
	...
	e(1)=0.0d0
        ...
        write(*,*)"the eigen values are:"
        j1=5
        do i=j1,j1+4
        write(*,*)d(i) 
        end do 
        open(10,file="tq_3.txt",status="unknown")

        do i=1,n
        write(10,*)x(i),z(i,j1),z(i,j1+1),z(i,j1+2),z(i,j1+3),v(x(i)) 
        end do 
        stop 
        end 
...
c ------ just change this function and the x-limits accordingly
	a=1.0
	if((x-10.0d0).gt.-a .and. (x-10.0d0).lt.a) then
	V=01.0
	else 
	V=0.0 
	end if 
...
C     Computes sqrt(A**2 + B**2) without destructive underflow or overflow
      ...
      ELSE IF (ABSB .EQ. 0.0D0 .AND. ABSA .eq. 0.0d0) THEN
         PYTHAG = 0.0D0
      ELSE
